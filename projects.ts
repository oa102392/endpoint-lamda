...
export default async function handler(
    ...
    const projects = <Project[]>(await client.query(
        'select project_index, project_funding_start, project_funding_end, current_trl, site, program from sandbox.investment where project_index is not null and project_index != 259'
    )).rows
